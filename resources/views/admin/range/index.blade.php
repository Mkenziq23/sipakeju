            ...
            <div class="card card-primary">
                <div class="card-header">
                    ...
                    <div class="card-header-action">
                        <a href="{{ route('admin.range.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i>
                            Tambah</a>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <table class="table table-striped table-bordered" id="tabel">
                                <thead>
                                    <tr>
                                        ...
                                        {{-- <th>Tingkat Kecenderungan Perilaku Judi Online</th> --}}
                                        <th width="10%">CF</th>
                                        <th width="17%">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($ranges as $range)
                                        <tr>
                                            ...
                                            <td>{{ $range->keterangan }}</td>
                                            <td>{{ $range->min_value }} - {{ $range->max_value }}</td>
                                            <td>
                                                <a class="btn btn-icon btn-warning btn-sm"
                                                    href="{{ route('admin.range.edit', $range->id) }}">
                                                    <i class="fa fa-edit"></i> Ubah
                                                </a>
                                                <form action="{{ route('admin.range.destroy', $range->id) }}" method="POST"
                                                    id="delete_{{ $range->id }}" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
