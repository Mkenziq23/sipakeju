...
<body>
    ...
    @foreach ($diagnosa as $d)
        <div class="title"
            style="font-size: 20px; font-weight: bold; text-align: center; padding: 10px; background-color: #f1f1f1; border: 2px solid #ccc; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            {{ $judul }}
        </div>

        <h3 style="text-align: center; font-size: 16px; font-weight: normal; color: #555; margin-top: 5px;">
            <span style="font-weight: bold;">Periode:</span> {{ $awalPeriode }} s/d {{ $akhirPeriode }}
        </h3>
        ...
        <table>
            ...
            <tbody>
                @foreach (json_decode($d->kondisi, true) as $gejalaId => $kondisiItem)
                    <tr>
        ...
        <table>
            <tr>
                ...
                <td>{{ number_format($d->presentase, 2) }}%</td>
            </tr>
            <tr>
                <th>Tingkat Kecenderungan</th>
                <td>{{ $d->tingkat_kecenderungan }}</td>
            </tr>
        </table>
